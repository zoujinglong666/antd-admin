...
const contentCls = computed ( () => {
  ...
  if ( contentWidth.value === 'Fluid' )
    ...
    cls.push ( ...['max-w-1600px w-1600px', 'mx-auto'] )
...
<template>
  <div>
    <div class="bg-[var(--bg-color)]" :class="layoutSetting.multiTab ? 'pb-16px' : 'py-16px'" px-24px mb-24px mx--24px
         mt--24px>
      ...
      <div flex mt-8px justify-between>
        <div flex items-center my-4px of-hidden>
          <slot name="title">
            <span text-20px line-height-32px mr-12px mb-0 truncate
                  font-600>{{ renderTitle ( title ?? currentItem.title ) }}</span>
          </slot>
        </div>
        <div>
          <slot name="extra"/>
